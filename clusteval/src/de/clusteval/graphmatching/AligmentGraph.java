...
public class AligmentGraph extends DirectedSparseMultigraph<String, String> {
	...
	public AligmentGraph(GraphMatching graphMatching, DirectedSparseMultigraph<String, String> g1, DirectedSparseMultigraph<String, String> g2){
		...
		Collections.sort(this.connectedComponents, new Comparator<Set<String>>() {
	...
	public double getEdgeCorrectness(){
		return this.getEdgeCount()/(double)Math.min(graphOne.getEdgeCount(), graphOne.getEdgeCount());
	}
	...
	public int getCompactness(int l){
		...
		UnweightedShortestPath<String, String> pathsLengthsOne = new UnweightedShortestPath<String, String>(graphOne); 
		UnweightedShortestPath<String, String> pathsLengthsTwo = new UnweightedShortestPath<String, String>(graphTwo);
		
		for (String v : matching.getMatchingForGraphOneVertices() ) {
			
			
			KNeighborhoodFilter<String, String> kneighborhoodFilterOne = new KNeighborhoodFilter<String, String>(v, l,  KNeighborhoodFilter.EdgeType.IN_OUT);
			Graph<String, String> nlv = kneighborhoodFilterOne.transform(graphOne);
			
			for (String u: nlv.getVertices()){
				if (matching.getMatchingForGraph1Vertex(u) != null){
					Number distOne = pathsLengthsOne.getDistance(v, u);
					Number distTwo = pathsLengthsTwo.getDistance(matching.getMatchingForGraph1Vertex(v), matching.getMatchingForGraph1Vertex(u));
					
					if (distTwo != null){
						properDistance = distOne.intValue();
						imageDistance = distTwo.intValue();
						compactnessDist += Math.max(imageDistance-properDistance, 0);
					}
				}
